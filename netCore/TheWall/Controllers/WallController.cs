...
namespace TheWall.Controllers
{
    public class WallController : Controller
    {
        ...
        public IActionResult Wall()
        {
            ...
            List<Dictionary<string,object>> AllMessages = _dbConnector.Query("Select messages.id As message_id, message, Messages.created_at, Messages.updated_at, first_name from Messages JOIN users ON users.id = messages.users_id ORDER BY Messages.created_at DESC;");
            System.Console.WriteLine("All Messages", AllMessages);
            ViewBag.AllMessages = AllMessages;
            List<Dictionary<string, object>> QueryComments = _dbConnector.Query("Select comments.id AS comment_id, comments.messages_id, users.first_name, comments.created_at, comments.comment from comments JOIN messages ON comments.messages_id = messages.id JOIN users ON comments.users_id = users.id Order By comments.created_at DESC;");
